    ...
    return (
        <div className={'DetailHeader'}>
            <button onClick={()=>{ //수정 조건: 한글 2, 3글자 & 학년은 1, 2, 3중 하나
                const korean = /[가-힣]/;
                if( 1<props.changedData.name.length && props.changedData.name.length<4 && korean.test(props.changedData.name[0]) && korean.test(props.changedData.name[0]) &&  (props.changedData.grade === 1 || props.changedData.grade === 2 || props.changedData.grade === 3 )  ) {
