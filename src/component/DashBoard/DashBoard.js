...
const DashBoard = () => {
  ...
  useEffect(() => {
    ...
    const interValid = setInterval(() => {
      axios.get("/student/stat").then((response) => {
        setDashBoardTotalData(response.data.count);
      });
    }, 3000);
      ...
      {context.studentData.length === 0 ? (
        ...
        <span className={styles.DashBoardContent}>
          ...
          <table className={styles.StudentHowMuch}>
            <thead>
              <tr>
                <th className={styles.StudentTableH} scope={"cols"}>
                  1학년
                </th>
                <th className={styles.StudentTableH} scope={"cols"}>
                  2학년
            ...
            <tbody>
              <tr>
                <td className={styles.StudentTableD}>
                  {dashBoardTotalData["1"]}
                </td>
                <td className={styles.StudentTableD}>
                  {dashBoardTotalData["2"]}
                </td>
                <td className={styles.StudentTableD}>
                  {dashBoardTotalData["3"]}
                </td>
              </tr>
            </tbody>
          </table>
