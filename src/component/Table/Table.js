...
const Table = (props) => {
    ...
    const TableContents = context.studentData
        .filter((student) => params.get('name') === '' || params.get('name') === null || student.name.includes(params.get('name')))
        .filter((student) => params.get('grade') === '' || params.get('grade') === null || student.grade === Number(params.get('grade')))
        .map((student) => {
            return (
